...
parser.add_argument('--bits', default='16', type=str,
                    help='binary code length (default: 16,32,48,64,128)')
parser.add_argument('--gpu', default='2', type=str,
                    help='selected gpu (default: 2)')
...
def calc_loss(W, L, V, U, G, S, S_1, code_length, select_index):
    ...
    l2 = W * W
    loss = (square_loss.sum() + label_loss.sum() + square_loss_1.sum() + l2.sum()) / (opt.num_samples * num_database)
    return loss
...
def epah_algo(code_length):
    ...
    model_txt.cuda()
    adsh_loss = dl.EPAHLoss(code_length, num_database)
    adsh_loss_txt = dl.EPAHLoss(code_length, num_database)
    ...
    for iter in range(max_iter):
        ...
        barU[select_index, :] = U
        barG[select_index, :] = G
        Q = -2 * code_length * Sim.cpu().numpy().transpose().dot(U + G) - 2 * database_labels.numpy().dot(W.transpose())
        for k in range(code_length):
            ...
            U_ = U[:, sel_ind]
            G_ = G[:, sel_ind]
            V[:, k] = -np.tanh(Q[:, k] + 2 * V_.dot(U_.transpose().dot(Uk) + 2 * G_.transpose().dot(Gk)
                                                    + 2 * W_.transpose().dot(Wk)))

        I = np.eye(code_length)
        P = np.matrix(U.transpose().dot(U)) + np.matrix(G.transpose().dot(G))\
            + np.matrix(V.transpose().dot(V)) + I
        PN = np.linalg.pinv(P)
        BL = (barU + barG + V).transpose().dot(database_labels.numpy())
        W = np.asarray(PN.dot(BL))

        lossx = calc_loss(W, sample_label, V, U, G, Sim.cpu().numpy(), S1.cpu().numpy(), code_length, select_index)
